...
- 数据文件：**136 个 JSON**（含 KET 备考 + PET 镜像 + exam 清单 + V0.6 语法增强 + 语法大厅 42 课）
- 语法大厅：**42/50 课**（基石 G01-G12 + 骨架 G13-G26 + 进阶 G27-G42 全部完成，共 2688 题 + 295 侦探病句，题干全局无重复；精修 G43-G50 待 P4）
...
- 勋章：20 个；Service Worker 缓存版本：**ea-v0.9.4**（壳预缓存架构；0.9.2 为并行会话覆写产生的倒退号，已更正，见环境坑 9）
