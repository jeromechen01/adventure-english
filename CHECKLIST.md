...
- ⚠️ `data/grammar/index.json` 里各课的 `ketLessonMap` 是写课时的参考标注，与本表**不完全一致**（G10→L8、G17→L2、G21→L1、G25→L1+L3 四处，本表未收）。跳转一律以映射模块为准；index.json 原样未动，是否对齐留给家长定
...
- 数据文件：**120 个 JSON**（含 KET 备考 + PET 镜像 + exam 清单 + V0.6 语法增强 + 语法大厅 26 课）
- 语法大厅：**26/50 课**（基石层 G01-G12 + 骨架层 G13-G26 全部完成，共 1664 题 + 183 侦探病句；进阶/精修待 P3-P4）
...
- 勋章：20 个；Service Worker 缓存版本：**ea-v0.9.25**（壳预缓存架构；0.9.2 为并行会话覆写产生的倒退号，已更正，见环境坑 9）
